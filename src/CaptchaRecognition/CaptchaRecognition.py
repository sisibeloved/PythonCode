...
def train_crack_captcha_cnn():
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            print("步长: {}  损失: {}".format(step, loss_))

            # 每10 step计算一次准确率
            if step % 10 == 0:
                batch_x_test, batch_y_test = get_next_batch(100)
                acc = sess.run(accuracy, feed_dict={X: batch_x_test, Y: batch_y_test, keep_prob: 1.})
                print("步长: {}  准确率: {}".format(step, acc))
                # 如果准确率大于50%,保存模型,完成训练
                if acc > 0.50:
                    print("训练完成！")
...
def crack_captcha(captcha_image):
    ...
    saver = tf.train.Saver()
    with tf.Session() as sess:
        saver.restore(sess, "./model/crack_captcha.model-1470")
...
if __name__ == '__main__':
    # 定义train变量,train=0:训练,train=1:预测
    train = 1
    if train == 0:
    ...
    if train == 1:
        ...
        print("正确值: {}  预测值: {}".format(text, predict_text))
