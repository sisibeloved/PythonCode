# 参考链接 http://cuiqingcai.com/3256.html
...
class ProxyHandler(object):
    """
     使用代理增加文章阅读量(类似于 CSDN 的博客里面的阅读数)
     """
    ...
    def __init__(self):
        ...
        # 目标地址
        self.visit_url = 'https://sisibeloved.github.io/hexoBlog'
        # 获取能用的代理集合
        self.proxy_list = []
        # 请求超时时间为3s
        self.timeout = 3
    ...
    def visit_url_by_proxy(self, url, timeout, proxy=None, num_retries=6, sleep_time=2):
        ...
        if proxy is None:  # 没有使用代理的时候
            try:
                response = requests.get(url=url, headers=headers, timeout=timeout)
                status_code = response.status_code
                print('随机产生的代理是====%s,返回的状态码是===%d' % (proxies, status_code))
            except:
                if num_retries > 0:  # num_retries 是我们限定的重试次数
                    time.sleep(sleep_time)  # 延迟十秒
                    print('获取网页出错，10S后将获取倒数第：', num_retries, '次')
                    return self.visit_url_by_proxy(url, timeout, num_retries - 1)  # 调用自身 并将次数减 1
                else:
                    print('开始使用代理')
